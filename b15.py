	...
	if msg.text :
		if msg.text == "/start":
			if msg.chat.type == "private" :
				if redis.sismember("T_Bot_Stats:Users",msg.from_user.id) != True:
		...
		if msg.chat.id in admins :
			...
			if msg.text == "متن خوش آمد گویی" :
				WelCome = (redis.hget("T_Bot","WelCome_Message") or "`تنظیم نشده`")
				Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def Set_Welcome_Text(msg) :
	if msg.text != "بازگشت" :
		redis.hset("T_Bot","WelCome_Message",msg.text) 
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		Button.add(types.KeyboardButton("آمار"),types.KeyboardButton("پیغام همگانی"))
		Button.add(types.KeyboardButton("متن خوش آمد گویی"),types.KeyboardButton("متن گروه جدید"))
...
def Set_New_GP_Text(msg) :
	if msg.text != "بازگشت" :
		redis.hset("T_Bot","New_Gp_Message",msg.text) 
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		Button.add(types.KeyboardButton("آمار"),types.KeyboardButton("پیغام همگانی"))
		Button.add(types.KeyboardButton("متن خوش آمد گویی"),types.KeyboardButton("متن گروه جدید"))
...
def Create_Banner(msg) :
	if msg.text :
		redis.hset('T_Bot:Create_Banner',"Type","Text")
		redis.hset('T_Bot:Create_Banner',"TEXT",msg.text)
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		...
		redis.hset('T_Bot:Create_Banner',"Type","Photo")
		redis.hset('T_Bot:Create_Banner',"File_ID",msg.photo[2].file_id)
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		Button.add(types.KeyboardButton("زیرنویس نمیخواهم"),types.KeyboardButton("بازگشت"))
		z = send(msg.from_user.id,"زیرنویس را ارسال کنید :\n\nنکته : اگر زیرنویس نمیخواهید دکمه 'زیرنویس نمیخواهم' را ارسال کنید",reply_markup=Button)
		bot.register_next_step_handler(z, Create_Banner_Photo_s2)
	elif msg.video :
		redis.hset('T_Bot:Create_Banner',"Type","Video")
		redis.hset('T_Bot:Create_Banner',"File_ID",msg.video.file_id)
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		Button.add(types.KeyboardButton("زیرنویس نمیخواهم"),types.KeyboardButton("بازگشت"))
		z = send(msg.from_user.id,"زیرنویس را ارسال کنید :\n\nنکته : اگر زیرنویس نمیخواهید دکمه 'زیرنویس نمیخواهم' را ارسال کنید",reply_markup=Button)
		bot.register_next_step_handler(z, Create_Banner_Video_s2)
	elif msg.documnet :
		redis.hset('T_Bot:Create_Banner',"Type","Doc")
		redis.hset('T_Bot:Create_Banner',"File_ID",msg.documnet.file_id)
		Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
		Button.add(types.KeyboardButton("زیرنویس نمیخواهم"),types.KeyboardButton("بازگشت"))
		z = send(msg.from_user.id,"زیرنویس را ارسال کنید :\n\nنکته : اگر زیرنویس نمیخواهید دکمه 'زیرنویس نمیخواهم' را ارسال کنید",reply_markup=Button)
...
def Create_Banner_Text_s2(msg) :
	if msg.text != "بازگشت" :
		if msg.text == "ارسال بدون دکمه" :
			q = redis.hget('T_Bot:Create_Banner',"TEXT")
			for i in redis.smembers("T_Bot_Stats:All") :
				try:
					send(i,q)
				except:
...
def Create_Banner_Photo_s2(msg):
	if msg.text != "بازگشت" :
		if msg.text == "زیرنویس نمیخواهم" :
			...
			redis.hset('T_Bot:Create_Banner',"Caption",msg.text)
			Button = types.ReplyKeyboardMarkup(resize_keyboard=True)
			Button.add(types.KeyboardButton("ارسال بدون کیبورد"),types.KeyboardButton("بازگشت"))
			z = send(msg.from_user.id,"کیبورد اینلاین را ارسال کنید :\n\nنکته : اگر کیبورد ندارید دکمه 'ارسال بدون کیبورد' را ارسال کنید",reply_markup=Button)
...
def Create_Banner_Photo_s3(msg) :
	if msg.text != "بازگشت" :
		if msg.text == "ارسال بدون کیبورد" :
			c = redis.hget('T_Bot:Create_Banner',"Caption")
			q = redis.hget('T_Bot:Create_Banner',"File_ID")
			for i in redis.smembers("T_Bot_Stats:All") :
				try:
			...
			markup = types.InlineKeyboardMarkup()
			ph = redis.hget('T_Bot:Create_Banner',"File_ID")
			q = redis.hget('T_Bot:Create_Banner',"Caption")
			for i in msg.text.split('\n') :	
				try:
					markup.add(types.InlineKeyboardButton(i.split("|")[0],url=i.split("|")[1]))
				except:
...
def Add(msg) :		
	if str(msg.new_chat_member.id) == str(BOT):
		redis.sadd("T_Bot_Stats:Groups",msg.chat.id)
		redis.sadd("T_Bot_Stats:All",msg.chat.id)
@bot.message_handler(content_types=['left_chat_member'])
def Left(msg) :		
	if str(msg.left_chat_member.id) == str(BOT):
		redis.srem("T_Bot_Stats:Groups",msg.chat.id)
		redis.srem("T_Bot_Stats:All",msg.chat.id)
